import { useState, useRef } from "react";
import { motion } from "framer-motion";

const EMAIL = "sidhaya.work@gmail.com";

function LinkedinIcon({ size = 18 }) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill="currentColor">
      <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
    </svg>
  );
}

function GithubIcon({ size = 18 }) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill="currentColor">
      <path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12" />
    </svg>
  );
}

function TelegramIcon({ size = 18 }) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill="currentColor">
      <path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z" />
    </svg>
  );
}

function InstagramIcon({ size = 18 }) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill="currentColor">
      <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm6.406-11.845a1.44 1.44 0 1 0 0 2.881 1.44 1.44 0 0 0 0-2.881z" />
    </svg>
  );
}

const fadeUp = {
  hidden: { opacity: 0, y: 30 },
  show: {
    opacity: 1,
    y: 0,
    transition: { duration: 0.7, ease: [0.22, 1, 0.36, 1] },
  },
};

const container = {
  hidden: {},
  show: { transition: { staggerChildren: 0.12 } },
};

export default function Contact() {
  const btnRef = useRef(null);
  const loaderRef = useRef(null);
  const checkRef = useRef(null);
  const textRef = useRef(null);

  const animateEl = (el, props, duration) =>
    new Promise((res) => {
      if (!el) return res();
      Object.assign(el.style, props);
      setTimeout(res, duration * 1000);
    });

  const handleEmailClick = async () => {
    navigator.clipboard.writeText(EMAIL);
    window.open(`mailto:${EMAIL}`, "_self");

    // Show loader
    await animateEl(textRef.current, { opacity: "0", width: "0", overflow: "hidden" }, 0.15);
    await animateEl(loaderRef.current, { display: "block", opacity: "1", width: "32px" }, 0.15);
    await new Promise(r => setTimeout(r, 1500));

    // Show check
    await animateEl(loaderRef.current, { opacity: "0", width: "0", display: "none" }, 0.15);
    await animateEl(checkRef.current, { display: "block", opacity: "1", width: "32px" }, 0.15);
    await new Promise(r => setTimeout(r, 2000));

    // Reset
    await animateEl(checkRef.current, { opacity: "0", width: "0", display: "none" }, 0.15);
    await animateEl(textRef.current, { opacity: "1", width: "auto", overflow: "visible" }, 0.15);
  };

  return (
    <section id="contact" className="relative flex flex-col min-h-screen px-6 pt-32 pb-12 md:pt-40 md:pb-16">
      <div className="mx-auto max-w-3xl w-full flex flex-col items-center gap-[75px] md:gap-[100px]">
        {/* ── Heading ── */}
        <motion.div
          variants={fadeUp}
          initial="hidden"
          whileInView="show"
          viewport={{ once: true, margin: "-80px" }}
          className="text-center"
        >
          <h2 className="text-3xl font-bold tracking-tight text-light-text dark:text-dark-text sm:text-4xl md:text-5xl">
            Let&rsquo;s Build Something
          </h2>
          <div className="mx-auto mt-4 h-1 w-12 rounded-full bg-accent" />
        </motion.div>

        {/* ── Contact CTA ── */}
        <div className="flex flex-col items-center gap-8 md:gap-4 w-full">
          <motion.div
            variants={container}
            initial="hidden"
            whileInView="show"
            viewport={{ once: true, margin: "-60px" }}
            className="flex flex-col items-center gap-8 w-full"
          >
            <motion.div variants={fadeUp} className="relative group">
              <button
                ref={btnRef}
                onClick={handleEmailClick}
                className="inline-flex items-center gap-4 rounded-full purple-gradient-border font-semibold text-light-text dark:text-dark-text transition-all duration-300 hover:shadow-[0_0_32px_rgba(168,85,247,0.4)] hover:scale-[1.03] active:scale-[0.97] px-12 py-5 text-lg md:text-2xl cursor-pointer"
                style={{
                  textDecoration: "none",
                }}
              >
                {/* Loader spinner — hidden by default */}
                <svg
                  ref={loaderRef}
                  style={{ display: "none", opacity: 0, width: 0 }}
                  xmlns="http://www.w3.org/2000/svg"
                  width="32" height="32" viewBox="0 0 24 24"
                  fill="none" stroke="#A855F7" strokeWidth="2.5"
                  strokeLinecap="round" strokeLinejoin="round"
                  className="animate-spin"
                >
                  <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                  <path d="M12 3a9 9 0 1 0 9 9" />
                </svg>

                {/* Check icon — hidden by default */}
                <svg
                  ref={checkRef}
                  style={{ display: "none", opacity: 0, width: 0 }}
                  xmlns="http://www.w3.org/2000/svg"
                  width="32" height="32" viewBox="0 0 24 24"
                  fill="none" stroke="#A855F7" strokeWidth="2.5"
                  strokeLinecap="round" strokeLinejoin="round"
                >
                  <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                  <path d="M12 12m-9 0a9 9 0 1 0 18 0a9 9 0 1 0 -18 0" />
                  <path d="M9 12l2 2l4 -4" />
                </svg>

                {/* Email text */}
                <span ref={textRef} className="text-light-text dark:text-dark-text flex items-center gap-3">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="24" height="24"
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="2"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    className="w-5 h-5 md:w-7 md:h-7 opacity-70 group-hover:opacity-100 transition-opacity"
                  >
                    <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                    <path d="M8 7a4 4 0 1 0 8 0a4 4 0 0 0 -8 0" />
                    <path d="M6 21v-2a4 4 0 0 1 4 -4h4a4 4 0 0 1 4 4v2" />
                  </svg>
                  {EMAIL}
                </span>
              </button>
            </motion.div>

            {/* Social icons */}
            <motion.div variants={fadeUp} className="flex flex-col items-center gap-6 pb-4 mt-12">
              <span className="text-[15px] uppercase tracking-[0.3em] text-black dark:text-white font-bold">Or reach out directly</span>
              <div className="flex items-center gap-6">
                <a
                  href="https://github.com/siddd247"
                  target="_blank"
                  rel="noopener noreferrer"
                  id="contact-github"
                  className="flex h-14 w-14 items-center justify-center rounded-2xl border border-light-text/20 dark:border-dark-text/20 md:border-light-text/8 md:dark:border-dark-text/8 bg-white dark:bg-zinc-900 text-black dark:text-white transition-all duration-300 hover:border-light-text dark:hover:border-dark-text hover:text-light-text dark:hover:text-dark-text hover:scale-110 active:scale-110"
                >
                  <GithubIcon size={26} />
                </a>
                <a
                  href="https://linkedin.com/in/sidhaya-katoch247"
                  target="_blank"
                  rel="noopener noreferrer"
                  id="contact-linkedin"
                  className="flex h-14 w-14 items-center justify-center rounded-2xl border-black/10 dark:border-white/10 md:border-light-text/8 md:dark:border-dark-text/8 border bg-white dark:bg-zinc-900 text-[#0077B5] sm:text-black sm:dark:text-white transition-all duration-300 hover:border-[#0077B5] hover:text-[#0077B5] hover:scale-110 active:scale-110"
                >
                  <LinkedinIcon size={26} />
                </a>
                <a
                  href="https://t.me/SIDDD247"
                  target="_blank"
                  rel="noopener noreferrer"
                  id="contact-telegram"
                  className="flex h-14 w-14 items-center justify-center rounded-2xl border-black/10 dark:border-white/10 md:border-light-text/8 md:dark:border-dark-text/8 border bg-white dark:bg-zinc-900 text-[#2AABEE] sm:text-black sm:dark:text-white transition-all duration-300 hover:border-[#2AABEE] hover:text-[#2AABEE] hover:scale-110 active:scale-110"
                >
                  <TelegramIcon size={26} />
                </a>
                <a
                  href="https://www.instagram.com/sidhaya.katoch/"
                  target="_blank"
                  rel="noopener noreferrer"
                  id="contact-instagram"
                  className="flex h-14 w-14 items-center justify-center rounded-2xl border-black/10 dark:border-white/10 md:border-light-text/8 md:dark:border-dark-text/8 border bg-white dark:bg-zinc-900 text-[#E1306C] sm:text-black sm:dark:text-white transition-all duration-300 hover:border-[#E1306C] hover:text-[#E1306C] hover:scale-110 active:scale-110"
                >
                  <InstagramIcon size={26} />
                </a>
              </div>
            </motion.div>
          </motion.div>
        </div>
      </div>
    </section>
  );
}
